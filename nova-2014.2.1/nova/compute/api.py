...
class API(base.Base):
    ...
    def _provision_instances(self, context, instance_type, min_count,
            ...
            instance_groups, check_server_group_quota):
        # Reserve quotas
    ...
    #MOD by zhjh 2015-02-27 modify the method for migrate relativeDispersionGroup from icehouse.
    @staticmethod
    def _get_requested_instance_group(context, scheduler_hints,
        ...
        relative_group_hint = scheduler_hints.get('relativeDispersionGroup')

        if not group_hint and not relative_group_hint:
            return

        instance_groups = list()
        if group_hint:
            instance_groups.append({'group_hint': group_hint,
                                'group_policies': scheduler_hints.get('GroupPolicies'),
                                'group_type': 'group'})
        if relative_group_hint:
            instance_groups.append({'group_hint': relative_group_hint,
                                'group_policies': scheduler_hints.get('RelativeGroupPolicies'),
                                'group_type': 'relativeDispersionGroup'})

        for instance_group in instance_groups:
            if uuidutils.is_uuid_like(instance_group['group_hint']):
                group = objects.InstanceGroup.get_by_uuid(context, instance_group['group_hint'])
            else:
                try:
                    group = objects.InstanceGroup.get_by_name(context, instance_group['group_hint'])
                    ...
                    group.name = instance_group['group_hint']
                    group.project_id = context.project_id
                    group.user_id = context.user_id
                    if instance_group['group_policies'] and isinstance(instance_group['group_policies'],list):
                        group.policies = instance_group['group_policies']
                    else:
                        group.policies = ['anti-affinity']
#                    group.metadetails = {"group_tye": instance_group['group_type']}
                    try:
                        group.create()
                    except Exception:
                        with excutils.save_and_reraise_exception():
    ...
    def _create_instance(self, context, instance_type,
        ...
        instance_groups = self._get_requested_instance_group(context,
    ...
    def resize(self, context, instance, flavor_id=None, host=None,
        ...
        #ADD by zhjh migrate or resize instance to selected host. 2015-02-13
        if (host == instance['node']) and ((not flavor_id and not CONF.allow_migrate_to_same_host)
                     or (flavor_id and not CONF.allow_resize_to_same_host)):
            msg = "Not allow to select the same host."
        ...
        if flavor_id and not CONF.allow_resize_to_same_host:
            filter_properties['ignore_hosts'].append(instance['host'])
        elif flavor_id: #ADD zhjh for resize same host 2015-02-13
        ...
        #ADD by zhjh migrate or resize instance to selected host. 2015-02-13
        if host:
            filter_properties['force_nodes'] = [host]
