...
class ResourceTracker(object):
    ...
    def _update_usage_from_instances(self, context, resources, instances):
        ...
        extra_stats = dict(resources.get('stats') or {})
        resources['stats'] = {}
        ...
        if extra_stats:
            if resources['stats']:
                resources['stats'] = dict(simplejson.loads(resources['stats']), **extra_stats)
            else:
                json_string=simplejson.dumps(resources['stats'])
                resources['stats'] = dict(simplejson.loads(json_string), **extra_stats)

            resources['stats'] = jsonutils.dumps(resources['stats'])
        if isinstance(resources['stats'], dict):
            resources['stats'] = jsonutils.dumps(resources['stats'])
