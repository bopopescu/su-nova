...
class _RelaGroupAntiAffinityFilter(AffinityFilter):
    """Schedule the instance on a different host from a set of group
    hosts.
    """

    def __init__(self):
        super(_RelaGroupAntiAffinityFilter, self).__init__()

    def host_passes(self, host_state, filter_properties):
        # Only invoke the filter is 'anti-affinity' is configured
        policies = filter_properties.get('relative_group_policies', [])
        if self.policy_name not in policies:
            return True

        group_hosts = filter_properties.get('relative_group_hosts') or []
        LOG.debug("Group anti affinity: check if %(host)s not "
                    "in %(configured)s", {'host': host_state.host,
                                           'configured': group_hosts})
        if group_hosts:
...
class ServerRelaGroupAntiAffinityFilter(_RelaGroupAntiAffinityFilter):
    def __init__(self):
        self.policy_name = 'anti-affinity'
        super(ServerRelaGroupAntiAffinityFilter, self).__init__()
