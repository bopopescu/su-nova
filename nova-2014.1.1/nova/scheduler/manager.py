...
class SchedulerManager(manager.Manager):
    ...
    #Add by zhjh for allocation ratio 2014-11-05
    def get_allocation_ratio(self, context, cpu=True, mem=True, disk=True):
        allocation_ratio = {}
        if cpu:
            allocation_ratio['cpu'] = CONF.cpu_allocation_ratio
        if mem:
            allocation_ratio['mem'] = CONF.ram_allocation_ratio
        if disk:
            allocation_ratio['disk'] = CONF.disk_allocation_ratio
...
class _SchedulerManagerV3Proxy(object):
    ...
    #Add by zhjh for allocation ratio 2014-11-05
    def get_allocation_ratio(self, ctxt, cpu=True, mem=True, disk=True):
        return self.manager.get_allocation_ratio(ctxt, cpu, mem, disk)
