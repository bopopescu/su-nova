...
class FilterScheduler(driver.Scheduler):
    ...
    def schedule_run_instance(self, context, request_spec,
        ...
        if not weighed_hosts:
            ...
            group_uuid = scheduler_hints.get('relativeDispersionGroup')
            if group_uuid:
                scheduler_hints.pop('relativeDispersionGroup')
                filter_properties.pop('relative_group_hosts')
    ...
    def _populate_retry(self, filter_properties, instance_properties):
        ...
        scheduler_hints = filter_properties.get('scheduler_hints') or {}
        group_uuid = scheduler_hints.get('relativeDispersionGroup', None)
        
        if retry['num_attempts'] > max_attempts and group_uuid:
            scheduler_hints.pop('relativeDispersionGroup', None)
            filter_properties.pop('relative_group_hosts', None)
            retry = {
                'num_attempts': 1,
                'hosts': []  # list of compute hosts tried
            }
    ...
    def _setup_instance_group(context, filter_properties):
       ...
       #14060857 add for relative group scheduler hints
    @staticmethod
    def _setup_instance_relative_group(context, filter_properties):
        update_group_hosts = False
        scheduler_hints = filter_properties.get('scheduler_hints') or {}
        group_hint = scheduler_hints.get('relativeDispersionGroup', None)
        if group_hint:
            group = instance_group_obj.InstanceGroup.get_by_hint(context,
                    group_hint)
            policies = set(('anti-affinity', 'affinity'))
            if any((policy in policies) for policy in group.policies):
                update_group_hosts = True
                filter_properties.setdefault('relative_group_hosts', set())
                user_hosts = set(filter_properties['relative_group_hosts'])
                group_hosts = set(group.get_hosts(context))
                filter_properties['relative_group_hosts'] = user_hosts | group_hosts
                filter_properties['relative_group_policies'] = group.policies
        return update_group_hosts

    #14060857 add for relative group
    def _schedule(self, context, request_spec, filter_properties,
        ...
        update_group_hosts = self._setup_instance_group(context,
                filter_properties)
        update_relative_group_hosts = self._setup_instance_relative_group(context,
                filter_properties)
        ...
        for num in xrange(num_instances):
            ...
            # because relative_group_hosts key may be removed from filter_properties in 
            # _populate_retry when retry times up to max
            if update_relative_group_hosts is True and 'relative_group_hosts' in filter_properties.keys():
                filter_properties['relative_group_hosts'].add(chosen_host.obj.host)
