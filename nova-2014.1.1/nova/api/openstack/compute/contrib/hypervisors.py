...
def make_hypervisor(elem, detail):
    ...
    if detail:
        ...
        elem.set('host_ip')
        elem.set('cinder_vgs_used')
        elem.set('cinder_vgs_total')
...
class HypervisorsController(object):
    ...
    def _view_hypervisor(self, hypervisor, detail, servers=None, **kwargs):
        ...
        if detail and not servers:
            ...
            stats = hypervisor.get('stats', None)
            if stats:
                hyp_dict['cinder_vgs_used'] = eval(stats).get('cinder_vgs_used', 0)
                hyp_dict['cinder_vgs_total'] = eval(stats).get('cinder_vgs_total', 0)
    ...
    def index(self, req):
        ...
        ratio = self.scheduler_rpcapi.get_allocation_ratio(context)
    ...
    def detail(self, req):
        ...
        #Add by zhjh for allocation ratio 2015-02-12
	ratio = self.scheduler_rpcapi.get_allocation_ratio(context)
