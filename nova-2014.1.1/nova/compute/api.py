...
class API(base.Base):
    ...
    @staticmethod
    def _update_instance_group_by_name(context, instance_uuids, group_name, group_policies=None, group_type="group"):
        try:
            ...
            if group_policies and isinstance(group_policies,list):
                ig.policies = group_policies
            else:
                ig.policies = ['anti-affinity']
            ig.members = instance_uuids
            ig.metadetails = {"group_tye": group_type}
            ig.create(context)

    #14060857 add for scheduler hint: relative group
    @staticmethod
    def _update_instance_relative_group(context, instances, scheduler_hints):
        ...
        if not relative_group_hint:
            return

        instance_uuids = [instance.uuid for instance in instances]

        if uuidutils.is_uuid_like(relative_group_hint):
            instance_group_obj.InstanceGroup.add_members(context, relative_group_hint,
                    instance_uuids)
        else:
            API._update_instance_group_by_name(context, instance_uuids,
                    relative_group_hint, group_policies, "relativeDispersionGroup")
    ...
    def _update_instance_group(context, instances, scheduler_hints):
        ...
        if uuidutils.is_uuid_like(group_hint):
            ...
            API._update_instance_group_by_name(context, instance_uuids,
                    group_hint, group_policies, "group")
    ...
    def resize(self, context, instance, flavor_id=None, host=None,
        ...
        #ADD by zhjh 2014-12-03 migrate or resize instance to selected host.
        ...
        elif flavor_id: #ADD zhjh 2014-11-06 for resize same host
